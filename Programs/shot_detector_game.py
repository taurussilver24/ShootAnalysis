...
class ShotDetector:
    def __init__(self, model_path, video_name, model_name, monitor_idx=0):
        ...
        monitor_info = outputs[monitor_idx]
        print(f"[INFO] Using monitor {monitor_idx}: {monitor_info}")

        self.target_width = monitor_info['width']
        self.target_height = monitor_info['height']
        self.capture_region = (
            monitor_info['left'],
            monitor_info['top'],
        ...
        while True:
            if not self.paused:
                try:
                    ...
                    # Verify and enforce correct resolution
    ...
    def process_frame(self, frame):
        # Run detection at full native resolution
        results = self.model(
            frame,
            stream=True,
            verbose=False,
            imgsz=(self.target_height, self.target_width),
            half=True,
            device='0',
            conf=0.75
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--monitor', type=int, default=1,
                        help="Monitor index to capture (default 1 = secondary monitor)")
